...
变量$v.dis$​记录了源点$s$​到节点$v$​最短路径的长度. 证明如下: 记源点$s$​到节点$v$​的最短路径长度为$\delta(s,v)$​​,它有如下性质:
  ...
  * **引理5.1** 对于有向或无向图$G$, 对任意边$uv$​, 有$\delta(s,v) \leq \delta(s,u)+1$​​.
    ...
    如果源点$s$到$u$可达, 则由于$uv$的存在, $s$到$v$同样可达. 所以$s$到$v$最短路径长度必然不超过任意一条$s$到$v$的路径长度(包括$s$到$u$再到$v$这条路径), 所以$\delta(s,v) \leq \delta(s,u)+1$.  若$s$到$u$不可达, 则$\delta(s,u)=\infty$, 不等式同样成立.
  ...
  * **引理5.2** 从节点$s$​开始BFS, 在遍历结束时, 对每个可达结点$v$​, 有 $v.dis \geq \delta(s,v)$​​.
    ...
    采用数学归纳法,对队列上的操作个数归纳, 即证明: 无论队列上执行了多少个操作, 下面的不变式总是成立: 对于任意$v$, $v.dis \geq \delta(s,v)$.
    ...
    $\quad \quad  = \delta(s,u)+1$ ( 归纳假设 )

    $\quad \quad \geq \delta(s,v)$​ ( 引理5.1 )
...
* 有向图
  ...
  * TE: 当遍历节点 $v$ 时，发现其白色邻居 $v$,  则 $uv$​​​​​ 为TE.   在一个连通片内所有TE组成的子图**连通**且**无环**. 如果忽略所有边的方向, 则这些TE组成当前连通片的一棵生成树,称为"BFS树"
  * BE: 当遍历节点$u$时,发现其黑色邻居 $v$, 且 $v$是$u$在BFS树中的祖先节点, 则$uv$​为BE. 对于BE $uv$, 有 $ 1 \leq v.dis < u.dis$​​.

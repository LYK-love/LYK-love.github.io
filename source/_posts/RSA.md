...
RSA是最著名的公钥密码体制, 由美国三位科学家Rivest，Shamir和Adleman于1976年提出, 并在1978年正式发表的RSA. 它基于数论中的大数分解问题.
...
我们已经计算出了$z = \phi(n)$ , 但是计算 $\phi[\phi(n)]$ 还是挺慢的, 可以另一种方法:
...
1. 被加密的消息 $m$ 需要是一个的整数( 我们可以将任意字节流直接解读为一个无符号整数 ), 满足 $ m < n $ . 如果消息太大，解读为整数以后比 n 要大，那么分段加密即可. 
...
所以， **$d$ 的安全性依赖于对  $n$ 进行质数分解的难度**. 目前来说，大整数的质数分解是一件相当困难的事情，参考 [整数分解](https://zh.wikipedia.org/wiki/整数分解)
